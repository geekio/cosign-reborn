...
f_login( SNET *sn, int ac, char *av[], SNET *pushersn )
{
	...
	syslog( LOG_ERR, "f_login: open 1 tmppath: %s: %m", tmppath );
	return( -1 );
    }
	...
	syslog( LOG_ERR, "f_login: open 1 krbpath: %s: %m", krbpath );
	return( -1 );
    }
...
do_register( char *login, char *login_p, char *scookie_p )
{
    ...
    if (( fd = open( tmppath, O_CREAT|O_EXCL|O_WRONLY, 0644 )) < 0 ) {
	syslog( LOG_ERR, "do_register: open 2 tmppath: %s: %m", tmppath );
	return( -1 );
    }
    ...
    *p = '\0';
	...
	if ( rc == REG_NOMATCH ) {
	    syslog( LOG_ERR, "service_valid: CN %s not allowed "
			"access to cookie %s (no match), pattern=%s", remote_cn, service, sl->sl_auth->al_hostname ); // aom
	} else {
	    regerror( rc, &preg, buf, sizeof( buf ));
...
retr_ticket( SNET *sn, struct servicelist *sl, char *krbpath )
{
    ...
    if (( fd = open( krbpath, O_RDONLY, 0 )) < 0 ) {
        syslog( LOG_ERR, "open 2 krbpath: %s: %m", krbpath );
